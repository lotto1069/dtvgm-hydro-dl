# Hydro-DL Final Model - TVGM_v15.6 (Integrated with Importance Table Output)
#         high metrics, CSV results, prediction plots, and a detailed
#         driving factor importance table (both in console and as a CSV file).
#                 to print and save a table of driving factor importances.
...
VERSION = "TVGM_v15.6_Final_Complete_Run"
...
def prepare_and_engineer_features(data_folders, flow_path, start_date, end_date, set_name):
    ...
    return df_filled
def analyze_and_plot_feature_importance(model, X, y, feature_names, top_n=20):
    """
    """
    print("  - Analyzing feature importance using permutation_importance...")
    ...
    filtered_importance_df = importance_df[~importance_df['Feature'].str.startswith('flow')].copy()

    sorted_df = filtered_importance_df.sort_values(by='Importance', ascending=False).reset_index(drop=True)
    ...
    csv_path = os.path.join(output_folder, f"driving_factors_importance_{VERSION}.csv")
    sorted_df.to_csv(csv_path, index=False, encoding='utf-8-sig')

    top_features_to_plot = sorted_df.head(top_n)

    plt.figure(figsize=(12, 10))
    plt.title(f'Top {top_n} Driving Factors for TVGM Booster (Runoff Features Excluded)', fontsize=16)

    plt.barh(top_features_to_plot['Feature'][::-1],
             top_features_to_plot['Importance'][::-1],
    ...
    plt.savefig(f'feature_importance_no_runoff_{VERSION}.png', dpi=300)
    plt.close()
...
def train_and_predict_tvgm_v15(train_df, test_df, feature_cols, target_col):
    ...
    print("  - Step 3: Training a Gradient Boosting model to learn the complex residuals (Q_ml_boost)...")
    ...
    analyze_and_plot_feature_importance(ml_booster, train_df[feature_cols], ml_target, feature_cols)

    q_ml_boost_train = ml_booster.predict(train_df[feature_cols])
    q_ml_boost_test = ml_booster.predict(test_df[feature_cols])

    print("  - Step 4: Combining Q_base and Q_ml_boost for the final TVGM prediction...")
...
class ResidualTransformer(nn.Module):
    ...
    def forward(self, x): x = self.input_fc(x) + self.pos_encoder[:, :x.size(1), :]; return self.output_fc(
        self.transformer_encoder(x)[:, -1, :])
def create_sequences(input_data, target_data, seq_length):
    xs, ys = [], [];
    for i in range(len(input_data) - seq_length): xs.append(input_data[i:(i + seq_length)]); ys.append(
        target_data[i + seq_length])
...
def train_dl_model(model, train_loader, val_loader, epochs, lr, patience):
    ...
    for epoch in pbar:
        model.train();
        for seq, labels in train_loader: seq, labels = seq.to(DEVICE), labels.to(
            DEVICE); optimizer.zero_grad(); loss = criterion(model(seq), labels); loss.backward(); optimizer.step()
        model.eval();
        val_loss = 0
        with torch.no_grad():
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(),
                       f'best_{model.__class__.__name__}.pth')
        else:
            epochs_no_improve += 1
...
if __name__ == "__main__":
    ...
    dl_models_config = {
        ...
        'Transformer': {'model': ResidualTransformer(len(common_features), nhead=len(common_features) // 3 if len(
            common_features) % 3 == 0 else 1), 'params': {'epochs': 100, 'lr': 0.001, 'patience': 10}}
    }

    num_features = len(common_features)
    nhead = 1
    # d_model in Transformer is 64, it must be divisible by nhead.
    for i in [8, 4, 2, 1]:
        if 64 % i == 0:
            nhead = i
            break
    ...
    for name, config in dl_models_config.items():
        ...
        preds = []
        with torch.no_grad():
            for seq, _ in test_loader: preds.extend(model(seq.to(DEVICE)).cpu().numpy().flatten())
    ...
    ax.plot(plot_df.index, plot_df['TVGM_pred'], label=f'TVGM (Base, NSE={metrics[VERSION]["NSE_Test"]:.3f})',
            color='red', linestyle='--')
    colors = {'LSTM': 'blue', 'TCN': 'green', 'Transformer': 'purple'}
    for name in dl_models_config.keys():
        full_name = f'{VERSION}+{name}'
        # Check if the predictions exist before plotting
        if f'TVGM+{name}_pred' in plot_df.columns:
            ax.plot(plot_df.index, plot_df[f'TVGM+{name}_pred'],
                    label=f'{name}-Corrected (NSE={metrics[full_name]["NSE_Test"]:.3f})', color=colors[name])
