# Hydro-DL Final Model - TVGM_v15.6 (Final Bug Fix for TCN)
#         produce all desired outputs: high metrics, importance plot, CSVs.
...
def analyze_and_plot_feature_importance(model, X, y, feature_names, top_n=20):
    ...
    sorted_df = filtered_importance_df.sort_values(by='importance', ascending=False)
...
if __name__ == "__main__":
    ...
    nhead = 1
    for i in [8, 4, 2, 1]:
        if (num_features * 2) % i == 0:  # d_model (64) must be divisible by nhead
            if (64 % i == 0):
                nhead = i
                break
