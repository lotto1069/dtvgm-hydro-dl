# Hydro-DL Model Validation Script - Transfer Learning (Fine-tuning)
#         1. Pre-train on a reference station.
#         2. Fine-tune the model on each new station's data.
...
VERSION = "TVGM_v15.6_FineTuning_Validation"
...
OUTPUT_DIR = "finetuning_validation_results"
...
def train_or_finetune_dl_model(model, train_loader, val_loader, epochs, lr, patience, model_save_path):
    ...
    return model
def main_finetuning_validation():
    # ==========================================================
    # ==========================================================
    print("=" * 60);
    print("PART 1: Pre-training the Reference TVGM+LSTM Model");
    print("=" * 60)

    print("Step 1.1: Loading and engineering original training data...")
    ...
    target_col = 'flow'
    print(f"\n- Using {len(feature_cols)} features for reference model.")
    ...
    pre_trained_lstm = ResidualLSTM(len(feature_cols)).to(DEVICE)
    pre_trained_lstm = train_or_finetune_dl_model(
        pre_trained_lstm, train_loader, val_loader, epochs=100, lr=0.001, patience=10,
        model_save_path="pretrained_lstm.pth"
    )
    print("  - LSTM model pre-trained.")
    print("\nReference Model is Ready for Fine-tuning.\n")

    # ==========================================================
    # ==========================================================
    print("=" * 60);
    print("PART 2: Fine-tuning and Validating on New Stations");
    print("=" * 60)
    ...
    for station_name, flow_path in validation_stations.items():
        try:
            print(f"\n--- Processing Station for Fine-tuning: {station_name} ---")
            ...
            split_point = int(len(val_df) * 0.7)
            ...
            finetune_booster.fit(finetune_df[feature_cols], ml_target_finetune)  # Re-fit on new data
            ...
            finetune_lstm.load_state_dict(torch.load("pretrained_lstm.pth"))

            ft_train_loader = DataLoader(TensorDataset(torch.FloatTensor(X_ft_seq), torch.FloatTensor(y_ft_seq)),
                                         batch_size=16, shuffle=True)  # smaller batch size

            finetuned_lstm_model = train_or_finetune_dl_model(
                finetune_lstm, ft_train_loader, ft_train_loader,
                # use train set as validation for simplicity in fine-tuning
                epochs=30, lr=1e-4, patience=5, model_save_path=f"finetuned_lstm_{station_name}.pth"
            )
            ...
            finetuned_lstm_model.eval()
            res_lstm_pred = []
            with torch.no_grad():
                test_seq_tensor = torch.FloatTensor(X_test_seq).to(DEVICE)
                res_lstm_pred = finetuned_lstm_model(test_seq_tensor).cpu().numpy().flatten()
            ...
            print(f"  - Final Metrics for {station_name} on test set: NSE={nse:.4f}, R2={r2:.4f}, RMSE={rmse:.4f}")

            test_df['Predicted_Flow'] = pd.Series(final_pred_aligned, index=dates_aligned)
            results_csv_path = os.path.join(OUTPUT_DIR, f"predictions_{station_name}.csv")
            test_df[['flow', 'Predicted_Flow']].rename(columns={'flow': 'Observed_Flow'}).to_csv(results_csv_path,
                                                                                                 encoding='utf-8-sig')
    ...
    # ==========================================================
    print("\n" + "=" * 60);
    print("PART 3: Final Fine-tuning Summary");
    print("=" * 60)
    summary_df = pd.DataFrame(validation_results).T
    print(summary_df.to_string(float_format="%.4f"))

    summary_path = os.path.join(OUTPUT_DIR, "finetuning_summary.xlsx")
    summary_df.to_excel(summary_path)
    print("\nFine-tuning validation process finished successfully.")
if __name__ == "__main__":
    main_finetuning_validation()