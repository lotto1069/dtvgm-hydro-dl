# Hydro-DL Model Validation - Final Optimization with Feature Engineering
#         (seasonality and rolling averages) to provide richer information.
...
VERSION = "TVGM_v15.9_FeatureEngineering"
...
OUTPUT_DIR = "feature_engineering_results"
...
def train_or_finetune_dl_model(model, train_loader, val_loader, epochs, lr, patience, model_save_path):
    ...
    return model
def main_feature_engineering_run():
    # ----------------------------------------------------------
    # PART 1: RE-TRAIN THE REFERENCE MODEL WITH NEW FEATURES
    # ----------------------------------------------------------
    print("=" * 60);
    print("PART 1: Pre-training Model with NEW Advanced Features");
    print("=" * 60)
    print("Step 1.1: Loading and engineering original training data with new features...")
    ...
    target_col = 'flow'
    print(f"\n- Using {len(feature_cols)} features for reference model (now includes temporal features).")
    ...
    print("Step 1.4: Pre-training LSTM on TVGM residuals...")
    ...
    pre_trained_lstm = ResidualLSTM(len(feature_cols)).to(DEVICE)  # Input size automatically adjusts
    pre_trained_lstm = train_or_finetune_dl_model(
        pre_trained_lstm, train_loader, val_loader, epochs=100, lr=0.001, patience=10,
        model_save_path="pretrained_lstm_feat_eng.pth"
    )
    print("  - LSTM model pre-trained with new features.")
    print("\nFeature-Rich Reference Model is Ready for Fine-tuning.\n")

    # ----------------------------------------------------------
    # ----------------------------------------------------------
    print("=" * 60);
    print("PART 2: Intelligently Fine-tuning and Validating on New Stations");
    print("=" * 60)
    validation_results = {}
    for station_name, flow_path in validation_stations.items():
        try:
            print(f"\n--- Processing Station: {station_name} ---")
            config = FINETUNE_CONFIG.get(station_name);
            print(f"  - Applying strategy: {config}")
            ...
            if config['finetune']:
                split_point = int(len(val_df) * config['split_ratio']);
                finetune_df = val_df.iloc[:split_point];
                test_df = val_df.iloc[split_point:]
            else:
                test_df = val_df
            if config['finetune']:
                print("  - Fine-tuning is ENABLED for this station.")
                ...
                finetune_lstm = ResidualLSTM(len(feature_cols)).to(DEVICE);
                finetune_lstm.load_state_dict(torch.load("pretrained_lstm_feat_eng.pth"))
                if config['freeze_layers']:
                    print("    - Freezing the first LSTM layer...")
                    for name, param in finetune_lstm.named_parameters():
                        if 'lstm.weight_ih_l0' in name or 'lstm.weight_hh_l0' in name or 'lstm.bias_ih_l0' in name or 'lstm.bias_hh_l0' in name: param.requires_grad = False
                ft_train_loader = DataLoader(TensorDataset(torch.FloatTensor(X_ft_seq), torch.FloatTensor(y_ft_seq)),
                                             batch_size=16, shuffle=True)
                finetuned_lstm_model = train_or_finetune_dl_model(
                    finetune_lstm, ft_train_loader, ft_train_loader, epochs=config['epochs'],
                    lr=config['learning_rate'], patience=5, model_save_path=f"finetuned_lstm_{station_name}.pth"
                )
                final_booster = finetune_booster;
                final_lstm = finetuned_lstm_model
            else:
                print("  - Fine-tuning is DISABLED. Using pre-trained models directly.")
                final_booster = ml_booster;
                final_lstm = pre_trained_lstm
            ...
            final_lstm.eval()
            with torch.no_grad():
                test_seq_tensor = torch.FloatTensor(X_test_seq).to(DEVICE);
                res_lstm_pred = final_lstm(test_seq_tensor).cpu().numpy().flatten()
            final_pred_aligned = np.maximum(0, q_tvgm_test[SEQ_LENGTH:] + res_lstm_pred);
            ...
            plot_title_suffix = '(Fine-tuned)' if config['finetune'] else '(Pre-trained Only)'
            print(
                f"  - Final Metrics for {station_name} {plot_title_suffix}: NSE={nse:.4f}, R2={r2:.4f}, RMSE={rmse:.4f}")
            results_df = pd.DataFrame({'Observed_Flow': observed_aligned, 'Predicted_Flow': final_pred_aligned},
                                      index=dates_aligned);
            results_csv_path = os.path.join(OUTPUT_DIR, f"predictions_{station_name}.csv")
            results_df.to_csv(results_csv_path, encoding='utf-8-sig');
    ...
    # ----------------------------------------------------------
    print("\n" + "=" * 60);
    print("PART 3: Final Feature Engineering Summary");
    print("=" * 60)
    summary_df = pd.DataFrame(validation_results).T;
    print(summary_df.to_string(float_format="%.4f"))
    summary_path = os.path.join(OUTPUT_DIR, "feature_engineering_summary.xlsx");
    summary_df.to_excel(summary_path)
    print("\nFinal optimization process finished successfully.")
if __name__ == "__main__":
    main_feature_engineering_run()