...
gs = GridSpec(2, 2, figure=fig,
              left=0.08, right=0.91, top=0.92, bottom=0.12,
...
ax_a.tick_params(axis='x', pad=25)

ax_a.set_yticks([0.5, 1.0])
ax_a.set_yticklabels(["0.5", "1.0"], color="grey", fontsize=TICK_SIZE)
ax_a.set_ylim(0, 1.1)
ax_a.set_rlabel_position(180)

ax_a.set_title("Multi-Model Performance", fontweight='bold', pad=35, fontsize=TITLE_SIZE + 2)
ax_a.legend(loc='lower center', bbox_to_anchor=(0.5, -0.3), ncol=2, fontsize=LEGEND_SIZE, frameon=True,
            edgecolor='gray')
...
ax_b2.tick_params(axis='y', labelsize=TICK_SIZE)

ax_b.set_ylim(0, 1.15)
ax_b2.set_ylim(0, 0.35)

ax_b.set_title("Model Performance: $R^2$ and RMSE", fontweight='bold', pad=20, fontsize=TITLE_SIZE + 2)
...
ax_c_bg.set_title("Residual Distribution", fontweight='bold', pad=35, fontsize=TITLE_SIZE + 2)
...
for idx, (model, pred_full) in enumerate(models.items()):
    ...
    if idx >= 2:
        ax.set_xlabel("Residual", fontsize=LABEL_SIZE, fontweight='bold')
...
for i, (name, data_full) in enumerate(groups.items()):
    ...
    name_display = name.replace("DTVGM+", "DT+") if name != "Observed" and name != "DTVGM" else name
...
ax_d.set_title("Radial Runoff Distribution", fontweight='bold', pad=30, fontsize=TITLE_SIZE + 2)

fig.text(0.02, 0.93, '(a)', fontsize=TAG_SIZE, fontweight='bold')
fig.text(0.50, 0.93, '(b)', fontsize=TAG_SIZE, fontweight='bold')
fig.text(0.02, 0.50, '(c)', fontsize=TAG_SIZE, fontweight='bold')
fig.text(0.50, 0.50, '(d)', fontsize=TAG_SIZE, fontweight='bold')

plt.savefig("Final_BigFont_Layout_Adjusted_V2.png", dpi=300, bbox_inches='tight')
plt.show()