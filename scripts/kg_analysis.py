...
fig_ts, axes_ts = plt.subplots(3, 1, figsize=(14, 12), sharex=False)

for i, (name, df) in enumerate(extended_dfs.items()):
    ax = axes_ts[i]

    ax.plot(df.index, df['Observed'], color='black', linewidth=0.7, alpha=0.9, label='Observed')
    ax.plot(df.index, df['Predicted'], color='#D62728', linewidth=0.7, alpha=0.8, label='Predicted')

    ax.set_ylabel('Runoff ($m^3/s$)', fontsize=12, fontweight='bold')
    ax.set_title(station_labels[i], loc='left', fontsize=14, fontweight='bold')
    ...
    ax.text(0.015, 0.95, textstr, transform=ax.transAxes, fontsize=11, fontweight='bold', verticalalignment='top',
            bbox=props)
    if i == 0: ax.legend(loc='upper right', ncol=2, frameon=False, fontsize=11)

plt.tight_layout()
plt.savefig(os.path.join(desktop_path, 'Final_Extreme_Timeseries.png'), dpi=300)
...
for i, (name, df) in enumerate(extended_dfs.items()):
    ...
    ax.scatter(obs, pred, facecolors='#1f77b4', edgecolors='black', s=10, alpha=0.3, linewidths=0.3)
    ...
    ax.set_xlabel('Observed Runoff ($m^3/s$)', fontsize=12, fontweight='bold')
    ax.set_ylabel('Predicted Runoff ($m^3/s$)', fontsize=12, fontweight='bold')
    ax.set_title(station_labels[i], fontsize=14, fontweight='bold')
    ax.set_xlim([lim_min, lim_max])
    ax.set_ylim([lim_min, lim_max])
    ax.grid(True, linestyle='--', alpha=0.5)

    ax.text(0.05, 0.90, f"$R^2$ = {m['R2']:.3f}", transform=ax.transAxes, fontsize=12, fontweight='bold')
    ax.legend(loc='lower right', frameon=False, fontsize=11)

plt.tight_layout()
plt.savefig(os.path.join(desktop_path, 'Final_Extreme_Scatter.png'), dpi=300)
